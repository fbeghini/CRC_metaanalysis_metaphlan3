...
---
```{r include=FALSE}
knitr::opts_chunk$set(echo = FALSE, fig.path = 'CRC_metaanalysis_plots/', dev = c('png','svg'), warning = FALSE)
library(tidyverse)
library(magrittr)
...
dim(rarefied_mpa2_table)
dim(rarefied_mpa3_table)
...
summarise(wilcox = wilcox.test(count ~ study_condition)$p.value %>% signif(2))
```
...
#hnn_genefamilies <- fread('genefamilies_wide.tsv', nThread = 50)
